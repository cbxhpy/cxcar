...
public class SpecManagerTest extends SpringTestSupport {
	...
	public void testAdd(){
		...
		SpringTestSupport.jdbcTemplate.execute("truncate table js_specification_2");
		SpringTestSupport.jdbcTemplate.execute("truncate table js_spec_values_2");
		...
		ISpecManager specManager = SpringTestSupport.getBean("specManager");
	...
	public void assertAdd(){
		...
		int count  = SpringTestSupport.jdbcTemplate.queryForInt(sql);
		assertEquals(count, 1);
		
		sql ="select count(0) from js_spec_values_2";
		count  = SpringTestSupport.jdbcTemplate.queryForInt(sql);
	...
	public void assertEdit(){
		...
		SpringTestSupport.jdbcTemplate.queryForInt(sql, new Object[]{"颜色1"});
		int count  = SpringTestSupport.jdbcTemplate.queryForInt(sql);
		assertEquals(count, 1);
		
		sql ="select count(0) from js_spec_values_2 where spec_name=?";
		count  = SpringTestSupport.jdbcTemplate.queryForInt(sql, new Object[]{"红色1"});
		assertEquals(count, 1);
	}
	...
	public void testValueList(){
		testAdd();
		ISpecValueManager specValueManager = SpringTestSupport.getBean("specValueManager");	
	...
	public void testGoodsSpecs(){
		IProductManager productManager = SpringTestSupport.getBean("productManager");
