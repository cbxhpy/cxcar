...
public class PromotionActivityTest extends SpringTestSupport {
	...
	@Before
	public void mock(){
		promotionActivityManager = SpringTestSupport.getBean("promotionActivityManager");
	...
	private void clean(){
		...
		SpringTestSupport.jdbcTemplate.execute("drop table if exists js_promotion_activity_2");
//		shopSaasDBRouter.createTable( "promotion_activity");
	...
	public void testAdd(){
		...
		int count  = SpringTestSupport.jdbcTemplate.queryForInt("select count(0) from js_promotion_activity_2 ");
		/**
		 * 验证数据库记录为1
		 */
	...
	public void testDelete(){
		...
		//验证规则 已清除		
		 count =  SpringTestSupport.jdbcTemplate.queryForInt("select count(0) from js_promotion_2");
		Assert.assertEquals(count, 0);
