...
public class PromotionTest extends SpringTestSupport {
	...
	@Before
	public void mock(){
		promotionManager = SpringTestSupport.getBean("promotionManager");
		shopSaasDBRouter = SpringTestSupport.getBean("shopSaasDBRouter");
	...
	private void clean(){
		...
		//促销表
		SpringTestSupport.jdbcTemplate.execute("drop table if exists js_promotion_2");
//		shopSaasDBRouter.createTable( "promotion");
		
		//促销会员关联表
		SpringTestSupport.jdbcTemplate.execute("drop table if exists js_pmt_member_lv_2");
//		shopSaasDBRouter.createTable( "pmt_member_lv");
		
		//促销商品关联表
		SpringTestSupport.jdbcTemplate.execute("drop table if exists js_pmt_goods_2");
//		shopSaasDBRouter.createTable( "pmt_goods");
	...
	public void testAddGiveGift(){
		...
		/**************验证*******************/
		//验证插入了一个规则
		int count =  SpringTestSupport.jdbcTemplate.queryForInt("select count(0) from js_promotion_2");
		Assert.assertEquals(count, 1);
		
		//验证关联了会员
		count =  SpringTestSupport.jdbcTemplate.queryForInt("select count(0) from js_pmt_member_lv_2");
		Assert.assertEquals(count,2);
		
		//验证未关联商品
		count =  SpringTestSupport.jdbcTemplate.queryForInt("select count(0) from js_pmt_goods_2");
