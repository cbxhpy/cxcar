	...
	public void onLogin(AdminUser user) {
			WebSessionContext<AdminUser>  sessonContext = ThreadContextHolder
				...
				Integer  depotid =(Integer)this.baseDaoSupport.queryForObject("select depotid from depot_user where userid=?", new IntegerMapper(),user.getUserid());
				 ...
				 stockUser.setProvince(user.getProvince());
				 stockUser.setCity(user.getCity());
				 stockUser.setRegion(user.getRegion());
				 stockUser.setProvince_id(user.getProvince_id());
				 stockUser.setCity_id(user.getCity_id());
				 stockUser.setRegion_id(user.getRegion_id());
				 if(depotid!= null)
