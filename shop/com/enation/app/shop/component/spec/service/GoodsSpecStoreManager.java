	...
	public int saveStore(int goodsid, String[] storeidAr, String[] productidAr,
		...
		for(int i= 0;i<storeidAr.length ;i ++){
			...
			if(storeid == 0) { //新库存
				...
				this.daoSupport.execute(" update es_product_store set enable_store = ? where storeid = ? ", store, storeid);
				this.daoSupport.execute(" update es_product_store set store= ?  where storeid = ? ", store, storeid);
			}
			
			//更新某个货品的总库存 
			this.daoSupport.execute("update es_product set store=(select sum(store) from es_product_store where productid=?),enable_store=(select sum(enable_store) from  es_product_store  where productid=?) where product_id=? ", productid,productid,productid);
			total+=store;
		}
		
		//更新商品总库存
		this.daoSupport.execute("update es_goods set store=(select sum(store) from es_product_store where goodsid=?),enable_store=(select sum(enable_store) from  es_product_store  where goodsid=?) where goods_id=? ",goodsid, goodsid,goodsid);
		return total;
