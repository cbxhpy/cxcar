...
public class ShopShortMsgPlugin extends AutoRegisterPlugin implements IShortMessageEvent{
	...
	private ShortMsg getDiscussMessage(){
		...
		StringBuffer sql = new StringBuffer(" select count(0) from es_member_comment emc ");
		sql.append(" left join es_seller es on es.seller_id = emc.seller_id ");
		sql.append(" where emc.type = 1 and emc.status = 0 ");
		...
		int count = this.baseDaoSupport.queryForInt(sql.toString());
		
		if(count>0){
			ShortMsg msg  = new ShortMsg();
			msg.setUrl("/shop/admin/comments!list.do?type=1&status_=0");
			msg.setTitle("评论列表");
			msg.setTarget("ajax");
