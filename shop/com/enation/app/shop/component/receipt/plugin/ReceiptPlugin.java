...
public class ReceiptPlugin extends AutoRegisterPlugin implements
		IAfterOrderCreateEvent,ICountPriceEvent {
	...
	public OrderPrice countPrice(OrderPrice orderprice) {
		...
		if(StringUtil.isEmpty(havereceipt)){
			return orderprice;
		} else{
			//需要发票，增加10快运费
			double needPayMoney = orderprice.getNeedPayMoney();
			double shippingPrice = orderprice.getShippingPrice();
			
			needPayMoney = CurrencyUtil.add(needPayMoney, 10d);
			
			orderprice.setShippingPrice(CurrencyUtil.add(shippingPrice, 10d));
