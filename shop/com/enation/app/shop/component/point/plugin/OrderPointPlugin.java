...
public class OrderPointPlugin extends AutoRegisterPlugin implements
		IAfterOrderCreateEvent,IOrderPayEvent,IOrderCanelEvent,IOrderRestoreEvent,IOrderReturnsEvent,ICountPriceEvent {
	...
	public void onAfterOrderCreate(Order order,List<CartItem>   itemList, String sessionid) {
		...
		if(order.getMember_id()!=null){
			...
			/**
			 * --------------------------------------
			 * 增加会员积分--商品价格*倍数(倍数在设置处指定) 消费红包PV*10元 (换算MP 单位分)
			 * --------------------------------------
			 */
...
//	/**
//	 * 已经发货10天之后的订单，则解冻佣金(从冻结佣金表删掉记录，将佣金增加到会员表，否则，光删除冻结积分表的记录即可)
//	 */移植至OrderStateCheckPlugin everyDay
//	@Override
//	public void everyDay() {
//		
//		//查出小当前日期 15天的冻结积分
//		List<FreezePoint> list = this.memberPointManger.listByBeforeDay(10);
...
//	}
	...
	public void restore(Order order) {
		//如果是会员购买发放积分
				if(order.getMember_id()!=null){
					 ...
					 * --------------------------------------
					 */
					if (memberPointManger.checkIsOpen(IMemberPointManger.TYPE_BUYGOODS) && member.getYongjin_flg()==1){
	...
	public OrderPrice countPrice(OrderPrice orderprice) {
		...
		Map map  = this.getGoodsPoint(sessionid);
		double point =Double.parseDouble(map.get("point").toString()); //商品本身的积分
		//double goodsprice = Double.parseDouble(map.get("price").toString()); //商品价格
