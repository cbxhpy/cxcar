...
//			strHtml+="<input type=\"submit\" name=\"submit\" value=\"提交到快钱\">";
	...
	public String onCallBack(String ordertype) {
		...
		logger.info("快钱 return-----------orderId----------"+orderId);
		//获取订单提交时间
		...
		logger.info("快钱 return---------payResult------------"+payResult);
		//获取错误代码
		///详细见文档错误代码列表
		...
		logger.info("快钱 return----------signMsg-----------"+signMsg);
		...
		if(signMsg.toUpperCase().equals(merchantSignMsg.toUpperCase())){
			...
			String url = "http://"+serverName+portstr+contextPath+"/s_billPlugin_payment-result.html" ;
			...
			switch(Integer.parseInt(payResult)){
				  ...
				  case 10:
					...
					this.paySuccess(orderId, dealId,ordertype);
					
					
					
					return "<result>1</result><redirecturl>" + url + "</redirecturl>";
