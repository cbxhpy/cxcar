...
public  class MemberMainWidget extends AbstractWidget { 
	...
	public String process(Map<String, String> params) {
		...
		if(
		   ...
		   &&!servletPath.equals("/member_logout.html")
		    ...
		    &&!servletPath.equals("/member_modifypassword.html")
		   &&member==null){
		...
		if( "yes".equals(ThreadContextHolder.getHttpRequest().getParameter("ajax") )){
			...
			if(showHtml){
				this.pageName= params.get("custom_page");
				this.folder= params.get("folder");
				if(!this.disableCustomPage && !StringUtil.isEmpty(pageName) ){
	...
	protected void execute(String mainHtml, Map<String, String> params) {
		...
		if(member!=null){
			this.putData("member", memberManager.get(member.getMember_id()));
...
//		this.putData("menuHtml", menuHtml);
		...
		//取未付款的订单数
		int needPayOrderNum = orderManager.getMemberOrderNum(member.getMember_id().intValue(), 0);
		this.putData("needPayOrderNum",needPayOrderNum);
