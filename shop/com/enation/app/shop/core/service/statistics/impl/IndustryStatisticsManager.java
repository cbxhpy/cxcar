 ...
 */
@Component
public class IndustryStatisticsManager extends BaseSupport implements
	...
	public List<Map> statistics_price(int type, int year, int month) {
		// cat_id 类型 name 名称
		List<Map> treeRoot = this.daoSupport.queryForList(treeRootSql);
		...
		for (int i = 0; i < treeRoot.size(); i++) {
			...
			if (type == 1) {
				...
				// 年份统计
				String temSql = "select SUM(oi.price) sump from es_order_items oi left join es_order od on oi.order_id = od.order_id where  oi.cat_id in (select cat_id from es_goods_cat gc where gc.cat_path like '0|"
						+ treeRoot.get(i).get("cat_id") + "%') and od.create_time BETWEEN "+this.getMinvalType0(year)+" and "+this.getMaxvalType0(year)+"  and "+this.getMaxvalType1(year, month) +" od.disabled = 0 AND ((od.payment_id = 3  AND od.status = "
				+ OrderStatus.ORDER_COMPLETE
				+ ") OR ( od.pay_status = "
				+ OrderStatus.ORDER_PAY_CONFIRM
				+ "))"; 
	...
	public List<Collect> listCollect(int cat_id,List<Cat> cats) { 
		...
		Date now = new Date();  
		Long endDate = DateUtil.getDateline(DateUtil.toString(now, null));
		...
		for (Cat cat : cats) { 
			...
			try {
				collect.setSaleroom(Double.parseDouble(map.get("zonge").toString()));
				collect.setSales(Integer.parseInt(map.get("zongs").toString()));
