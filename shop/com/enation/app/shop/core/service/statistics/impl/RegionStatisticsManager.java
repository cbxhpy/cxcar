...
public class RegionStatisticsManager implements IRegionStatisticsManager {
	...
	public List regionStatisticsList(Integer type, String sort,Integer cycle_type,Integer year,Integer month) {
		...
		sql.append("WHERE member_id IN(SELECT o.member_id from es_order o WHERE o.ship_provinceid=s.region_id AND o.`status`="+ OrderStatus.ORDER_COMPLETE);
	...
	private List getRegionOrderNum(Integer cycle_type,Integer year,Integer month) {
		StringBuffer sql = new StringBuffer("select s.local_name,(SELECT COUNT(o.order_id) FROM es_order o WHERE o.disabled=0 and o.ship_provinceid=s.region_id AND o.`status`="+ OrderStatus.ORDER_COMPLETE);
	...
	private List getRegionOrderPrice(Integer cycle_type,Integer year,Integer month) {
		StringBuffer sql = new StringBuffer("select s.local_name,(SELECT SUM(o.order_amount) from es_order o WHERE o.disabled=0 and o.ship_provinceid=s.region_id AND o.`status`="+ OrderStatus.ORDER_COMPLETE);
		//判断周期模式 1为月
		if(cycle_type==1){
			sql.append(" AND o.complete_time<"+getMaxvalType1(year,month)+" AND o.complete_time > "+getMinvalType1(year, month));
		}else{
	...
	private long getMaxvalType0(int year){ 
		 return DateUtil.getDateline(year+"-12-31");
	}
	/**
	 * 获取当前年份的 unix时间戳  最小值
	 * @param month
