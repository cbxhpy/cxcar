 ...
 */
@Component
public class SalesStatisticsManager extends BaseSupport implements ISalesStatisticsManager {
	...
	public List<MonthAmount> statisticsMonth_Amount() {
		...
		if(EopSetting.DBTYPE.equals("1")){//是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') as mo from " + this.getTableName("order") + " a where a.disabled=0 and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y') = ?  group by mo";
		}else if(EopSetting.DBTYPE.equals("3")){//是mssql
			sql = "select sum(a.order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) as mo from " + this.getTableName("order") + " a where a.disabled=0 and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,4) = ?  group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7)";
		}else{//是oracle
...
//			this.daoSupport.execute(createfunction);
	...
	public List<MonthAmount> statisticsMonth_Amount(String monthinput) {
		...
		if("1".equals(EopSetting.DBTYPE)){//是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') as mo from " + this.getTableName("order") + " a where a.disabled=0 and a.status = " + OrderStatus.ORDER_COMPLETE + " and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y') = ?  group by mo";
		}else if("2".equals(EopSetting.DBTYPE)){//是oracle
			sql = "select sum(a.order_amount) as amount, to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm') as mo from " + this.getTableName("order") + " a where a.disabled=0 and a.status = " + OrderStatus.ORDER_COMPLETE + " and to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy') = ?  group by to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm')";
		}else if("3".equals(EopSetting.DBTYPE)){//SQLServer
			sql = "select sum(order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) as mo from " + this.getTableName("order") + " where disabled=0 and status = " + OrderStatus.ORDER_COMPLETE + " and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ? group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7)";
		}
		List<Map> list = this.daoSupport.queryForList(sql, year);
		List<MonthAmount> target = new ArrayList<MonthAmount>();
	...
	public List<DayAmount> statisticsDay_Amount() {
		...
		String sql = "";
		if(EopSetting.DBTYPE.equals("1")){//是mysql
			sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m-%d') as mo from " + this.getTableName("order") + " a where a.disabled=0 and a.status = " + OrderStatus.ORDER_COMPLETE + " and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') = ?  group by mo";
		}else if(EopSetting.DBTYPE.equals("2")){
			sql = "select sum(a.order_amount) as amount, to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd') as mo from " + this.getTableName("order") + " a where a.disabled=0 and a.status = " + OrderStatus.ORDER_COMPLETE + " and to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm') = ?  group by to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd')";
		}else{
			sql = "select sum(a.order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from " + this.getTableName("order") + " a where a.disabled=0 and a.status = " + OrderStatus.ORDER_COMPLETE + " and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ?  group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
		}
		List<Map> list = this.daoSupport.queryForList(sql, year);
	...
	public List<DayAmount> statisticsDay_Amount(String monthinput) {
		...
		if("1".equals(EopSetting.DBTYPE)){//是mysql
			...
			sql = "select sum(order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from " + this.getTableName("order") + " where disabled=0 and status = " + OrderStatus.ORDER_COMPLETE + " and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ? group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
		}
	...
	public List<Map> statisticsYear_Amount(Integer status, int year) {
		...
		String sql =  "select count(0) as t_num,SUM(order_amount) as t_money,SUM(gainedpoint) as t_pv, case "+ condition_sql +" as month  from es_order o where o.disabled=0 ";
	 ...
	 */
	@Override
	public List getSalesIncome(int year, int month, int page, int pageSize,
			Map map) {
		...
		String sql = "select oi.goods_id,oi.`name`,oi.price,SUM(oi.num) t_num,SUM(oi.num*oi.price) t_price from es_order_items oi "
				...
				+ " where o.create_time >=? and  o.create_time <=?  group by oi.goods_id ";
	...
	public Double getReceivables(int year, int month, Map parames) {
		...
		String sql = "select SUM(o.order_amount) as receivables from es_order o where o.disabled=0 and create_time >=? and  create_time <=?";
	...
	public Double getRefund(int year, int month, Map parames) {
		...
		Map map = this.daoSupport.queryForMap(sql, start,end);
		Double refund = 0.0;
		if(map!=null && map.get("refund")!=null){
