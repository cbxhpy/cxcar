...
public class MemberStatisticsManager extends BaseSupport implements IMemberStatisticsManager {
	 ...
	 */
	@Override
	public List<Map<String, Object>> getWashPrice(String startDate, String endDate) {
		
		String sql = " SELECT SUM(pay_price) AS price, em.e_regtime AS washtime FROM "
				+ " (SELECT m.wash_record_id, m.pay_price, m.create_time, FROM_UNIXTIME(m.create_time/1000, '%d') AS e_regtime FROM es_wash_record m "
				+ " WHERE m.pay_status = 1 AND m.create_time >= "+startDate +" AND m.create_time <= "+endDate +") "
				+ " AS em GROUP BY em.e_regtime ";
	 ...
	 */
	@Override
	public List<Map<String, Object>> getYearWashPrice(String startDate, String endDate) {
		
		String sql = " SELECT SUM(pay_price) AS price, em.e_regtime AS washtime FROM "
				+ " (SELECT m.wash_record_id, m.pay_price, m.create_time, FROM_UNIXTIME(m.create_time/1000, '%m') AS e_regtime FROM es_wash_record m "
				+ " WHERE m.pay_status = 1 AND m.create_time >= "+startDate +" AND m.create_time <= "+endDate +") "
				+ " AS em GROUP BY em.e_regtime ";
	 ...
	 */
	@Override
	public List<Map<String, Object>> getRechargePrice(String startDate, String endDate) {
		
		String sql = " SELECT SUM(price) AS price, em.e_regtime AS rechargetime FROM "
				+ " (SELECT m.recharge_id, m.price, m.create_time, FROM_UNIXTIME(m.create_time/1000, '%d') AS e_regtime FROM es_recharge m "
				+ " WHERE m.pay_status = 1 AND m.create_time >= "+startDate +" AND m.create_time <= "+endDate +") "
				+ " AS em GROUP BY em.e_regtime ";
	 ...
	 */
	@Override
	public List<Map<String, Object>> getYearRechargePrice(String startDate, String endDate) {
		
		String sql = " SELECT SUM(price) AS price, em.e_regtime AS rechargetime FROM "
				+ " (SELECT m.recharge_id, m.price, m.create_time, FROM_UNIXTIME(m.create_time/1000, '%m') AS e_regtime FROM es_recharge m "
				+ " WHERE m.pay_status = 1 AND m.create_time >= "+startDate +" AND m.create_time <= "+endDate +") "
				+ " AS em GROUP BY em.e_regtime ";
