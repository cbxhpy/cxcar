...
//		this.updateDepotStore(orderId);
	...
	public void cancel(Integer orderId,String cancel_reason) {
		if(orderId== null) throw new IllegalArgumentException("param orderId is NULL");
		this.baseDaoSupport.execute(" update order set status = ?, cancel_reason = ? where order_id = ? ", OrderStatus.ORDER_CANCELLATION, cancel_reason, orderId);
	...
	@Override
	public void useOrder(Integer orderId) {
		if(orderId== null ) throw new  IllegalArgumentException("param orderId is NULL");
		this.daoSupport.execute(" update es_order set status = ?, use_time = ?  where order_id = ? ", OrderStatus.ORDER_ROG, DateUtil.getDateline(), orderId);
		this.log(orderId, "使用订单");
	}
	
	@Override
	public void commentOrder(Integer order_id) {
		if(order_id== null ) throw new  IllegalArgumentException("param orderId is NULL");
		this.daoSupport.execute(" update es_order set status = ? where order_id = ? ", OrderStatus.ORDER_COMPLETE, order_id);
		this.log(order_id, "评论订单");
	}
...
//			paymentLog.setPay_date(com.enation.framework.util.DateUtil.getDatelineLong());
		...
		paymentLog.setCreate_time(DateUtil.getDatelineLong());
		AdminUser adminUser  = this.adminUserManager.getCurrentUser();
		if(adminUser!=null){
...
//		}
	 ...
	 * 2016-10-27
	 * @param order_id 订单标识
	 * @param member_id 操作员标识
	 * @param op_name 操作员名称
	 * @param sign_name 签收人
	 * @param sign_time 签收时间
	 */
	@Override
	public void appRogConfirm(String order_id, Integer member_id, String op_name, String sign_name, Long sign_time){
		...
		//判断订单的付款状态是不是已付款，如果是已付款，则更新订单状态为完成
		//if(order.getPay_status() == OrderStatus.PAY_CONFIRM){
			orderstatus = OrderStatus.ORDER_COMPLETE;
			complete_time = DateUtil.getDatelineLong();
		//}
		
		//sale_cmpl=1 销售生效
		String sql = " update order set status="+orderstatus +", complete_time="+ complete_time +", ship_status="+OrderStatus.SHIP_ROG+",the_sign='"+sign_name+"',signing_time="+sign_time+" , sale_cmpl=1  where order_id = " + order_id;
		this.baseDaoSupport.execute(sql);
		
		//写入订单日志表
		this.log(Integer.parseInt(order_id), "确认收货成功", member_id, op_name);
		
		//写入会员购买商品表
		List<Map> itemList = this.orderManager.getItemsByOrderid(Integer.parseInt(order_id));
		for (Map map : itemList) {
			StringBuffer sql1 = new StringBuffer();
			sql1.append(" INSERT INTO member_order_item(member_id, goods_id, order_id, item_id, commented, comment_time) ");
			sql1.append(" VALUES(?, ?, ?, ?, 0, 0) ");
			this.baseDaoSupport.execute(sql1.toString(), order.getMember_id(),
				map.get("goods_id").toString(), map.get("order_id").toString(),
	...
	public void rogConfirm(int orderId,Integer op_id,String op_name,String sign_name,Long sign_time){
		...
		//sale_cmpl=1 销售生效
		String sql = " update order set status="+orderstatus +", complete_time="+ complete_time +", ship_status="+OrderStatus.SHIP_ROG+",the_sign='"+sign_name+"',signing_time="+sign_time+" , sale_cmpl=1  where order_id="+orderId;
		this.baseDaoSupport.execute(sql);
		//写入订单日志表
...
//		}
	...
	@Override
	public void appCancel(String order_id, String member_id){
		
		Order order = orderManager.get(Integer.parseInt(order_id));
		if(order == null){
			throw new RuntimeException("订单不存在");
		}
		...
		if(Integer.parseInt(member_id) != order.getMember_id()){
			throw new RuntimeException("没有操作此订单权限");
		}
		...
		this.log(order.getOrder_id(), "取消订单", member.getMember_id(), name);

	}
	
	@Override
	public void reminderDelivery(String order_id, String member_id) {
		...
		if(order.getIs_remind() == 1){
			throw new RuntimeException("已经提醒发货");
		}
		if(Integer.parseInt(member_id) != order.getMember_id()){
			throw new RuntimeException("没有操作此订单权限");
		}
		
		order.setIs_remind(1);
		orderManager.edit(order);
		
		Member member = this.memberManager.get(Integer.parseInt(member_id));
		String name = StringUtil.isEmpty(member.getName()) ? member.getUname() : member.getName();
		this.log(order.getOrder_id(), "提醒发货", member.getMember_id(), name);
		
	}
	
	@Override
	public void deleteOrder(String order_id, String member_id){
		...
		Member member = this.memberManager.get(Integer.parseInt(member_id));
		String name = StringUtil.isEmpty(member.getName()) ? member.getUname() : member.getName();
		this.log(order.getOrder_id(), "删除订单", member.getMember_id(), name);
