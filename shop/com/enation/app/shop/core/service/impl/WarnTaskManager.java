...
public class WarnTaskManager extends BaseSupport implements IWarnTaskManager {
	...
	public List<Map> listColor(Integer goodsId){
		String sql = "select  pc.productid,pc.color  from " + this.getTableName("product") + " p left join " + this.getTableName("product_color") + " pc  on p.product_id=pc.productid where p.goods_id="+goodsId;
	...
	public Page listAll(Integer page,Integer pageSize ) {
		String sql = "SELECT d.name as depotname,g.sn,g.name,gc.name as catname,w.* FROM " + this.getTableName("warn_task") + " w  "
				+" left join " + this.getTableName("goods") + " g on w.goodsid = g.goods_id "
				+" left join " + this.getTableName("goods_cat") + " gc on w.catid = gc.cat_id "
				+" left join " + this.getTableName("depot") + " d on d.id = w.depotid ";
	...
	public Page listdepot(Integer depotId,String name,String sn,int page,int pageSize) {
		...
		String sql = "select g.*,b.name as brand_name ,t.name as type_name,c.name as cat_name,wt.id as task_id,wt.productids,wt.sphere,wt.cylinder from "
	            ...
	            +" t on g.type_id =t.type_id  left join "+this.getTableName("warn_task") 
	            +" wt  on g.goods_id=wt.goodsid "
			    +" where wt.flag=1 and g.goods_type = 'normal' and g.disabled=0 and wt.depotid="+depotId;
	...
	public Map listTask(Integer taskId) {
		...
		return this.baseDaoSupport.queryForMap(sql, taskId);
	}
	@Override
	public Integer getProductId(Integer goodsid){
		String sql  = "select product_id from product where goods_id = ?";
		Integer productid  = this.baseDaoSupport.queryForInt(sql, goodsid);
		return productid;
	}
	@Override
	public void updateTask(Integer taskId) {
		this.baseDaoSupport
		.execute(
				"update warn_task set flag=2 where id=? ",
				taskId);
	}
