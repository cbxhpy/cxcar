...
//		int f = 1*60*60; //测试改成一个小时后 正是上线后将上面一行恢复即可
		...
		String sql ="select fp.*,o.status as order_status from "+this.getTableName("freeze_point")+"  fp inner join "+this.getTableName("order")+" o on fp.orderid= o.order_id  where  "+now+"-o.sale_cmpl_time>="+f;
		////System.out.println(sql);
