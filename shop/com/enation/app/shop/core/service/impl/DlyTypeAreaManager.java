...
public class DlyTypeAreaManager extends BaseSupport implements
		IDlyTypeAreaManager {

	@Override
	public Map listAllByRegion(String regionid) {
		String sql = "select dta.type_id,dta.area_id_group,dta.area_name_group,dta.has_cod,dta.config,dt.name,dt.is_same,dt.config as dt_config  from " + this.getTableName("dly_type_area") + " dta left join " + this.getTableName("dly_type") + " dt on dta.type_id = dt.type_id";
	...
	public int queryByrdgion(String regionid) {
		...
		if(!StringUtil.isEmpty(regionid)){
			cod = this.baseDaoSupport.queryForInt("select cod from regions where region_id = "+regionid);
		}
		return cod;
	}
