...
@Results({
	...
	@Result(name="order_pv", type="freemarker", location="/shop/admin/statistics/sales/order_pv.html"),
...
//		}
	 ...
	 * @return
	 */
	public String getSalePvJson(){
		
		String message ="[";
		if(cycle_type.intValue()==1){
			List<Map> list= salesStatisticsManager.statisticsMonth_Amount(order_status, year, month);
			message+=getMessage(cycle_type, "t_pv", list);
		}else{
			List<Map> list= salesStatisticsManager.statisticsYear_Amount(order_status, year);
			message+=getMessage(cycle_type, "t_pv", list);
		}
