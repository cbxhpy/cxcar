	...
	public String select_condition(){
		...
		try{
			...
			if(pmtid!=null){ //编辑时使用库中读取的开始、结束时间
				...
				for(int i=0,len=lvList.size();i<len;i++){
	...
	public String save(){
		...
		try{
			...
			if(pmtid!=null){
				...
				if(this.activityid == null){ //优惠劵 已经不走这个逻辑
...
//					coupons.setPmt_id(pmtid);
			...
			if(this.activityid == null){ 
				...
				this.urls.put("返回此活动促销规则列表", "promotion!list.do?activityid="+promotion.getPmta_id());
			}
