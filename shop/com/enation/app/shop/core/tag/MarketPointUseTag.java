...
public class MarketPointUseTag extends BaseFreeMarkerTag {
	...
	protected Object exec(Map params) throws TemplateModelException {
		...
		String use_point =request.getParameter("use_point");//是否使用积分
		
		if( "yes".equals(use_point)){
			String use_point_num =request.getParameter("point_num");//本次要使用的积分数
			userpoint = StringUtil.toInt(use_point_num,0);
			ThreadContextHolder.getSessionContext().setAttribute("use_point_num", userpoint);//把使用的积分数放在session中
			
		}else if("clean".equals(use_point) || "no".equals(use_point)){
			ThreadContextHolder.getSessionContext().removeAttribute("use_point_num");//如果不使用则清除
			userpoint =0;
		}
