package com.enation.app.shop.core.netty.omen.netty.server.codec;
 ...
 *        长度调节值，在总长被定义为包含包头长度时，修正信息长度，比如如果总长包含包头长，则该字段填-lengthFieldLength
...
public abstract class NettyMsgDecoder extends LengthFieldBasedFrameDecoder{
	 ...
	 * 
	 * @param byteOrder 
	 *         指定大小端解码方式，默认情况下一般为TCP/IP的大端模式，与C,C++通信时可能出现小端情况。 ByteOrder.BIG_ENDIAN;ByteOrder.LITTLE_ENDIAN
	   @param maxFrameLength
	 *         信息最大长度
	 * @param lengthFieldOffset
	 *         长度属性的起始（偏移）位,长度属性从哪个字节开始
	 ...
	 *        需要跳过的字节数
	 */
	public NettyMsgDecoder(ByteOrder byteOrder,int maxFrameLength, int lengthFieldOffset,
			int lengthFieldLength,int lengthAdjustment, int initialBytesToStrip) {
		super(byteOrder,maxFrameLength, lengthFieldOffset, lengthFieldLength,lengthAdjustment,initialBytesToStrip,true);
		marshallingDecoder = MarshallingCodeCFactory.buildMarshallingDecoder();
	}
