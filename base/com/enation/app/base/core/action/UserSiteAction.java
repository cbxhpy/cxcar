...
public class UserSiteAction extends WWAction {
	...
	public String backup() {
		try {
			name = "backup_" + DateUtil.getDatelineLong();
	...
	public String export() {
		try {
			this.solutionExporter.export(name, exportData == 1,	exportTheme == 1, exportProfile == 1, exportAttr == 1);
			this.json = "{result:1,path:'" + EopSetting.IMG_SERVER_DOMAIN + EopContext.getContext().getContextPath() + "/backup/" + name + ".zip" + "'}";
		} catch (RuntimeException e) {
			e.printStackTrace();
	...
	public String initData() {
		try {
			...
			Object realCode = sessonContext.getAttribute(ValidCodeServlet.SESSION_VALID_CODE + "initdata");

			if (!vcode.equalsIgnoreCase((String)realCode)) {
	...
	public String editSave() throws Exception {
		...
		try{
			if(!StringUtil.isEmpty(logopath)){
				logopath = logopath.replaceAll(EopSetting.IMG_SERVER_DOMAIN	+ EopContext.getContext().getContextPath(),	EopSetting.FILE_STORE_PREFIX);
				eopSite.setLogofile(logopath);
				eopSite.setBklogofile(logopath);
			}
			if(!StringUtil.isEmpty(iconpath)){
				iconpath = iconpath.replaceAll(EopSetting.IMG_SERVER_DOMAIN	+ EopContext.getContext().getContextPath(),	EopSetting.FILE_STORE_PREFIX);
				eopSite.setIcofile(iconpath);
			}
