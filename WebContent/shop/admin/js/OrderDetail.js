...
var OrderDetail = {
	...
	　　				 		}
		...
		// 使用订单绑定
		$("#useorder").unbind("click");
		$("#useorder").bind("click", function() {
			var disabled = $("#useorder").hasClass("l-btn-disabled");
			if(!disabled){
				if (confirm("确认要使用此订单吗？")) {
					self.useorder(self.orderid);
				}
			}
		});
		//发货
	...
	initBtnStatus : function() {
		...
		//待使用时才能使用
		if (this.orderStatus==OrderStatus.ORDER_SHIP) {
			$("#useorder").linkbutton("enable");
		} else {
			$("#useorder").linkbutton("disable");
		}
	 ...
	 * @param orderId
	 */
	useorder : function(orderId) {
		var self = this;
		$.Loading.show("正在保存请稍候");
		$("#useorder").linkbutton("disable");
		$.ajax({
			url : 'order!useOrder.do?ajax=yes&orderId='+ orderId,
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
					$.Loading.success(responseText.message);
