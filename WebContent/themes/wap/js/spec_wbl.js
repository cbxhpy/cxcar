...
Eop.SpecWbl={
	init:function(haveSpec){
		if(haveSpec==1){
			var self = this;
			this.refresh();
			$("#goods_spec .spec_item a[specvid!='']").live("click", function(){
				var link = $(this);
				if(link.attr("class")!='selected' && link.attr("class")!='disabled' ){
					self.specClick($(this));
					var goods_id = $(this).parent().parent().parent().parent().parent().parent().parent().attr("goods_id");
					var spec_value_id = $(this).attr("specvid");
					$(".my-gallery").empty();
					if(link.attr("spec_type") == '1'){
						$(".my-gallery").load("goods_spec_image_wbl.html?goods_id="+goods_id+"&spec_value_id="+spec_value_id);
					}
	...
	refresh:function(pro, specLink, product_ar){
		...
		if(product_ar.length>0){
			...
			$(".spec_item").not( specLink.parents(".spec_item") ).find("a").each(function(){
				...
				var result = Eop.SpecWbl.findProduct(proar);
...
var SelectTipRefresh={
	refresh:function(pro){
		...
		if(i>0){
			...
			specHtml="<em>请选择：</em><span>上方规格</span>";
		}
...
var PriceRefresh={
	refresh:function(pro){
		if(pro.length==1){
			//$(".goodprice h2 span").text("￥"+pro[0].price);
			$(".goodsprice").html("价格：￥"+pro[0].price);
			$(".store_num").html("库存："+pro[0].store);
			$(".spec_tip").hide();
			$("#productid").val(pro[0].product_id);
			$("#zongji").html(pro[0].price);
			$("#zongji").attr("store", pro[0].store);
			$("#havespec").val(1);
			$(".price_spec_num").show();
		}else{
			...
			//$(".goodprice h2 span").text("￥"+minPrice+"-￥" +maxPrice);
			$(".goodsprice").html("价格：￥"+minPrice+"-￥" +maxPrice);
			$(".store_num").show();
			$(".price_spec_num").show();
		}
	}
